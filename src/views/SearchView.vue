<template>
    <div class="flex flex-col min-h-screen bg-weather-primary">
      <Header :city="currentCity" :weather="currentWeather" :temperature="currentTemp" :wind="currentWind" @showInfo="showInfoModal" />
      
      <main class="w-3/4 mx-auto text-white flex flex-col items-center">
        <!-- City list -->
        <div class="w-full mt-4 max-h-[300px] overflow-y-scroll scrollbar-thin hover:scrollbar-thumb-blue-300">
          <div class="flex flex-col gap-2">
            <div v-for="city in cities" :key="city.name" class="mb-4 text-xl bg-weather-secondary py-4 px-5 flex justify-between cursor-pointer w-full duration-300">
              <h3>{{ city.name }}</h3>
        ...
        <div class="w-full bg-weather-secondary px-12 pt-10 mt-2 rounded">
          <div class="flex gap-6 text-lg" >
            <div v-for="day in recentWeather" :key="day.date" class="flex flex-col flex-1 text-center gap-4">
